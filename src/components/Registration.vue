  ...
  <div class="form-registration-block">
    <div class="form-registration">
      <div class="win-name-box">
        <div class="slot"></div>
        <div class="win-name"><h2>Регистрация</h2></div>
        <CloseButton />
      </div>
       ...
       <div class="if-box">
        ...
        <v-card-actions class="space-between">
          <button class="btn" @click="this.$router.go(-1)">Назад</button>
  ...
  methods: {
    onReg() {
      if (!this.password) {
        ...
        this.message = "Подтвердите пароль!"
      } else if (this.password.length < 8) {
        this.message = "Слишком короткий пароль!"
      } else if (this.password !== this.password_confirmation) {
        this.message = "Неверный повторный ввод пароля!"
      } else {
        const rand = Math.floor(Math.random() * 1e6)
        const hash = sha256(rand.toString()) + sha256(this.password)
        const signData = { login: this.name, email: this.email, pswd: hash }
        axios
          .post(host + '/signin', signData)
          .then(response => {
