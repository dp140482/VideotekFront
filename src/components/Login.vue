    ...
    <div class="form-registration">
      ...
      <div class="if-box">
       ...
       v-if="!token"
        ...
        ></v-text-field>
        <div class="btn-box">
          <button v-if="!token" value="login" class="btn" @click="onClick">Вход</button>
          <router-link v-if="!token" class="btn" :to="{ name: 'Registration' }">Регистрация</router-link>
          <div v-if="token" class="btn-row" >
            <button @click="onLogout" class="btn">Да</button>
            <button @click="$router.go(-1)" class="btn">Нет</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    onClick() {
      ...
      const hash = sha256(rand.toString()) + sha256(this.password)
      axios
        .post(host + '/login', { email: this.email, pswd: hash })
        .then(response => {
            if (response.data.auth) {
                localStorage.setItem('uid', response.data.auth)
    ...
    onLogout() {
      if (this.token) {
        axios
          .get(host + '/get-username/' + this.token)
          .then(response => {
            const userName = response.data.login
            axios
              .get(host + '/logout/' + userName)
              .then(response => {
